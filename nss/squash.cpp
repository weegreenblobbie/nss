    ...
    while (n < width) 
    {
        ...
        else if (m_direction == 1 && m > 0) 
        {
            for (int j = 0; j < size; ++j) 
            {
                ...
                window.pop(array(m0 + m - 1, n0 + j));
                // Push on new values from the new row (size - 1).
                window.push(array(m0 + m + size - 1, n0 + j));
            }
        } 
        ...
        else if (n > 0)
        {
            for (int i = 0; i < size; ++i)
            {
                ...
                window.pop(array(m0 + i, n0 + n - 1));
                // Push on new values from the column on the right (size - 1).
                window.push(array(m0 + i, n0 + n + size - 1));
            }
        }
        ...
        else 
        {
            ...
            {
                output(m, n) = pixel; //(pixel - min_val) / mag;
            }
        }
