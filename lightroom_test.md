...
* **LR Settings:** 
  ...
  * Balance/Blending = Default (usually 50)
...
* **LR Settings:**
  ...
  * *Note:* Since Lightroom doesn't have a single "Rotate" slider, manually calculate the offset: change Midtones to Hue = 90, and Shadows to Hue = 330 (which is 240 + 90). 
...
* **LR Settings:**
  * Set a heavy color cast (e.g., Midtones: Hue = 120, Saturation = 100).
  * Export one image with Balance = -100 (pushes Midtones into the shadows), and one with Balance = +100 (pushes Midtones into the highlights).
